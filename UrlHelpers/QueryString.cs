...
namespace SuperScript.ExternalFile.UrlHelpers
{
    ...
    public class QueryString : NameValueCollection
    {
        ...
        public QueryString FillFromString(string s)
        {
            ...
            foreach (var split in from keyValuePair in ExtractQuerystring(s).Split('&')
                                  where !string.IsNullOrEmpty(keyValuePair)
            ...
            for (var i = 0; i < base.Keys.Count; i++)
            {
                ...
                foreach (var val in base[base.Keys[i]].Split(','))
                    builder.Append((builder.Length == 0)
                                       ? "?"
                                       : "&").Append(HttpUtility.UrlEncodeUnicode(base.Keys[i])).Append("=").Append(val);
            }
